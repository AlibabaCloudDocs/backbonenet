...
![Routing policies](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/803311/156378436451040_en-US.png)
...
-   The basic policy information contains the following elements.
 ...
 |
    ...
    |**Direction**|The direction of the routing policy.     -   RegionIn: Indicates outbound routing from a region node.
    -   RegionOut: Indicates inbound routing to a region node.
 |
...
-   The matching conditions consist of the following elements:
 ...
 |
    |**A list of instance IDs**|The list of instance IDs contained in a matched route. The following instance IDs can be set as matching conditions:     -   A list of source instance IDs
    -   A list of target instance IDs
    -   A list of source and target instance IDs
 |
 ...
 |
    |**Route type**|The type of a matched route. The following route types can be set as matching conditions:     -   System routes: the routes automatically generated by the system
    -   User routes: the custom routes that you add manually
    -   BGP routes: the routes advertised to BGP
 |
    |**Route prefix**|The prefix of a matched route. The following matching modes are supported:     -   Fuzzy match: If the prefix of a route to be matched is contained in the prefix that is set as the matching condition, the route matches the condition.

For example, if the prefix in the matching condition is set as 1.1.0.0/16, and the matching mode is set to fuzzy match, the route with the prefix of 1.1.1.0/24 matches the condition.

    -   Exact match: A route matches the condition only when the prefix of the route is exactly the same as the prefix that is set as the matching condition.

For example, if the prefix in the matching condition is set as 1.1.0.0/16, and the matching mode is set to exact match, only the route with the prefix of 1.1.1.0/16 matches the condition.

 |
    |**AS Path**|The AS Path of a matched route. The following matching modes are supported:     -   Fuzzy match: A route matches the condition if the AS Path in the route overlaps with the AS Path that is set as the matching condition.

For example, if the AS Path in the matching condition is set as \[65001,65002\] and the matching mode is set to fuzzy match, the route with the AS Path of \[65501,65001\] matches the condition.

    -   Exact match: A route matches the condition only when the AS Path of the route is exactly the same as the AS Path that is set as the matching condition.

For example, if the AS Path in the matching condition is set as \[65501,65001,60011\] and the matching mode is set to exact match, only the route with the AS Path of \[65501,65001,60011\] matches the condition.
 ...
 **Note:** The AS Path is a well-known mandatory attribute, which describes the numbers of the ASs that a BGP route passes through during transmission.
 ...
 |
    |**Community**|The community of a matched route. The following matching modes are supported:     -   Fuzzy match: A route matches the condition if the community of the route overlaps with the community that is set as the matching condition.

For example, if the community in the matching condition is set as \[65001:1000,65002:2000\] and the matching mode is set to fuzzy match, the route with the community of \[65501:1000,65001:1000\] matches the condition.

    -   Exact match: A route matches the condition only when the community of the route is exactly the same as the community that is set as the matching condition.

For example, if the community in the matching condition is set as \[65001:65001,65002:65005,65003:65001\] and the matching mode is set to exact match, only the route with the community of \[65001:65001,65002:65005,65003:65001\] matches the condition.
...
![Matching process](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/803311/156378436451016_en-US.png)
...
-   Control the communication between two VPCs, or between a VPC and a VBR or CCN
    ...
    ![Control the communication between two VPCs, or between a VPC and a VBR or CCN ](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/803311/156378436551086_en-US.png)
...
-   Control the communication between two VBRs or between a VBR and a CCN
    ...
    ![Control the communication between two VBRs or between a VBR and a CCN](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/803311/156378436551104_en-US.png)
...
-   Control the communication between two CCNs or between a CCN and a VBR
    ...
    ![Control the communication between two CCNs or between a CCN and a VBR](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/803311/156378436551135_en-US.png)
